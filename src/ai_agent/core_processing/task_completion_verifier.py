...
class TaskCompletionVerifier:
    ...
    def verify_task_completion(self, original_instruction: str, session_id: Optional[str] = None) -> TaskVerification:
        ...
        try:
            ...
            verification_prompt = self._create_verification_prompt(
                original_instruction, save_logs
            )
    ...
    def _create_verification_prompt(self, original_instruction: str, save_logs: Dict[str, Any]) -> str:
...
1. Review the execution logs to understand what actions were taken
2. Identify any missing steps or incomplete actions
3. Assess confidence in completion (0.0 to 1.0)
...
Focus on the actual outcome of the executed commands versus the intended goal of the instruction."""
    ...
    def _call_gemini_for_verification(self, prompt: str) -> ModelResponse:
        """Call Gemini 3 Flash for task verification"""
        try:
            # Create API request for Gemini 3 Flash
            api_request = APIRequest(
                prompt=prompt,
                image_data=None,  # No image data for CLI verification
                image_format="PNG",
                max_tokens=5000,
